    ...
    public async buildNetwork() {
        try {
            await this.context.audioWorklet.addModule(
                'https://cdn.jsdelivr.net/gh/JohnVinyard/web-components@0.0.86/build/components/tanh.js'
            );
        } catch (err) {
        ...
        try {
            await this.context.audioWorklet.addModule(
                'https://cdn.jsdelivr.net/gh/JohnVinyard/web-components@0.0.86/build/components/attackenvelopes.js'
            );
        } catch (err) {
            console.log(`Failed to add module due to ${err}`);
            alert(`Failed to load module due to ${err}`);
